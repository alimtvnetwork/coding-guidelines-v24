...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/consolidated-install.ps1))) -Version v6.44.0 -Target .\vendor
#>
...
Write-InstallLog "# consolidated-install crash log"
...
$BundleMapping = "02-spec/01-spec-authoring-guide|02-spec/01-spec-authoring-guide,02-spec/03-error-manage|02-spec/03-error-manage,02-spec/17-consolidated-guidelines|02-spec/17-consolidated-guidelines,version.json|version.json,.ai-memory/what-to-read.md|.ai-memory/what-to-read.md,.ai-memory/memory|.ai-memory/memory,.ai-memory/coding-guidelines.md|.ai-memory/coding-guidelines.md,01-prompts|01-prompts,03-ai-scripts|03-ai-scripts,.ai-memory/plans|.ai-memory/plans,.ai-memory/issues|.ai-memory/issues,.ai-memory/cicd-issues|.ai-memory/cicd-issues,.agents/skills|.agents/skills,.agents/scripts|.agents/scripts,.ai-memory/strictly-avoid.md|.ai-memory/strictly-avoid.md"
...
Write-Host "  📦 consolidated-install v6.44.0" -ForegroundColor Cyan
...
function Copy-Mapping {
    ...
    try {
        ...
        $summaryData = [ordered]@{ bundle = "consolidated"; version = $finalGuidelineVersion; previousVersion = $prevVersion; installedAt = (Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"); filesInstalled = @($installedFilesSorted); filesRemoved = @($removedFilesSorted) }
...
Restore-CallerPreferences
    ...
    Write-Host "  ❌ consolidated-install failed (code $code)" -ForegroundColor Red
