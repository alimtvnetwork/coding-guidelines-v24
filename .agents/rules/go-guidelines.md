...
5. **Pointer-Attached Null Safety & Predicates:** Result inspection methods MUST attach to pointer receivers (`(r *Result[T])`) with line-1 `if r == nil` guards returning safe defaults. Callers use `IsCountOtherThan(N)`, `IsEmpty()`, `HasRecord()`, `IsDefined()`.
6. **Affirmative Boolean Parameters & Fields:** Never use single-letter boolean parameters (`v bool`, `b bool`) or bare verbs/nouns (`stop bool`, `pause bool`). Always use affirmative names: `isStopOnFail bool`, `isStopped bool`, `isPaused bool`.
