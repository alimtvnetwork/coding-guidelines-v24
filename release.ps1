param(
    [string]$Repo = "alimtvnetwork/coding-guidelines-v24"
)
...
function Invoke-BakeReleaseInstallers {
    ...
    $isShPlaceholder = (Get-Content $outSh -Raw) -match '__VERSION_PLACEHOLDER__'
    $isPs1Placeholder = (Get-Content $outPs1 -Raw) -match '__VERSION_PLACEHOLDER__'
    if ($isShPlaceholder -or $isPs1Placeholder) {
...
New-ReleaseArchives
Write-Step "Baking release-install scripts with VERSION_PLACEHOLDER -> v$version"
