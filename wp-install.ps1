...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/wp-install.ps1))) -Version v6.43.0 -Target .\vendor
#>
...
$Script:__InstallCrashLogFile = Join-Path $Script:__InstallCrashLogDir ("wp-install-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ") + ".log")
...
Write-InstallLog "# wp-install crash log"
...
$BundleName = "wp"
$BundleMapping = "02-spec/18-wp-plugin-how-to|02-spec/18-wp-plugin-how-to,version.json|version.json,.ai-memory/what-to-read.md|.ai-memory/what-to-read.md,.ai-memory/memory|.ai-memory/memory"
...
Write-Host "  📦 wp-install v6.43.0" -ForegroundColor Cyan
...
function Copy-Mapping {
    ...
    try {
        ...
        $summaryData = [ordered]@{ bundle = "wp"; version = $finalGuidelineVersion; previousVersion = $prevVersion; installedAt = (Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"); filesInstalled = @($installedFilesSorted); filesRemoved = @($removedFilesSorted) }
...
Restore-CallerPreferences
    ...
    Write-Host "  ❌ wp-install failed (code $code)" -ForegroundColor Red
