...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/linters-install.ps1))) -Version v6.44.0 -Target .\vendor
#>
...
$BundleName = "linters"
$BundleMapping = "linters|linters,linters-cicd|linters-cicd,version.json|version.json,.ai-memory/what-to-read.md|.ai-memory/what-to-read.md,.ai-memory/memory|.ai-memory/memory"
...
Write-Host "  📦 linters-install v6.44.0" -ForegroundColor Cyan
...
function Copy-Mapping {
    ...
    try {
        ...
        $summaryData = [ordered]@{ bundle = "linters"; version = $finalGuidelineVersion; previousVersion = $prevVersion; installedAt = (Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"); filesInstalled = @($installedFilesSorted); filesRemoved = @($removedFilesSorted) }
