...
19. **Version Source of Truth (Non-Negotiable)**: Do not hardcode version numbers in multiple files. There MUST be a `version.json` file in the root of the repo (delivered via the coding guidelines installer). It must contain the repository's root version information, description, and self-explaining metadata (`_purpose`, `_instructions`). Every language implementation (Go, TypeScript, Python, etc.) MUST import this single `version.json` file to get the repo's version. To make a new release or change the version across the entire repo, you MUST only change this root `version.json` and run the project's sync command (`npm run sync`).
