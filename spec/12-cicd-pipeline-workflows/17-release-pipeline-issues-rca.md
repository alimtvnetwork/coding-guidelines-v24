...
The following lessons were captured during the development of the a sibling reference implementation (its `spec/12-cicd-pipeline-workflows/10-known-issues-and-fixes.md` and `spec/16-generic-release/08-known-issues-and-fixes.md`). They describe failure modes that **will eventually occur in this repo too** if/when we add: a Go binary release, Windows resource embedding, SHA-deduplication, install-script generation, or multi-asset uploads. They are documented preemptively so the same diagnosis cycle does not repeat.
...
| Coding standards | `spec/02-coding-guidelines/01-index.md` |
