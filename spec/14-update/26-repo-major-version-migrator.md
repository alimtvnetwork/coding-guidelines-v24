...
When a major version is cut, the repo is renamed (e.g. `coding-guidelines-v18` → `coding-guidelines-v18`). Every URL, badge, install one-liner, `package.json` repo field, README link, and CI workflow that hardcodes the old name becomes stale. Manual replacement is error-prone and easy to miss in JSON, YAML, and markdown stamps.
...
|------|----------|---------|
| `--from <slug>` | yes | Old repo slug, e.g. `alimtvnetwork/coding-guidelines-v18`. |
...
5. If old slug appears inside a longer identifier (e.g. `coding-guidelines-v18-archive`), the script does NOT rewrite it and reports the collision.
