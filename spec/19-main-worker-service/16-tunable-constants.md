...
| Override at runtime = Seedable-Config Categories block (NOT `Tables`). | Per `spec/06-seedable-config-architecture/02-fundamentals.md`. |
...
- `spec/06-seedable-config-architecture/02-fundamentals.md` — `Categories` block schema (§4 conforms).
- `spec/19-main-worker-service/05-worker-routing.md` — consumer (FU-14).
- `spec/19-main-worker-service/11-worker-bootstrap-protocol.md` §3.2/§7 — consumer (FU-14).
- `spec/19-main-worker-service/13-jwt-delivery-contract.md` §6 — consumer (already cites tunables but pinned here).
- `spec/14-update/28-worker-push-instruction.md` §3/§7 — consumer (FU-14).
- `spec/19-main-worker-service/14-error-codes.md` — `WORKER-300-01` triggered when `IdempotencyKeyTtlSeconds` exceeded.
