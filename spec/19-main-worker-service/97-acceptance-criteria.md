...
| Uses split DB per `spec/05-split-db-architecture/` | `02-architecture.md` §2, `12-split-db-tier-reconciliation.md` §4, `diagrams/erd-worker-split-db.mmd` | Worker startup creates **Root/Settings/App/Session** DBs (4 tiers per FU-1; Cache + Document tiers unused in v1.0) |
...
| Sub-criterion | Defined in | Test |
|---------------|-----------|------|
| Endpoint, JSON instruction download, zip-based update flow documented | `10-self-update-pointer.md` §3 | File exists and references `spec/14-update/` |
