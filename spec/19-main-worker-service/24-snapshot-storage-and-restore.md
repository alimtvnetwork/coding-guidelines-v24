...
1. **Threat-model document** at `spec/19-main-worker-service/25-threat-model.md` (slot reserved by `13-jwt-delivery-contract.md` §11.3) — must enumerate the new attack surfaces (diff-chain corruption for §14.1; partial-restore plaintext extract for §14.2).
