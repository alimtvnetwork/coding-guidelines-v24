...
Provisioned by the installer (see `spec/14-update/19-install-scripts.md`) into the Worker's **Settings tier** DB:
...
- `spec/19-main-worker-service/05-worker-routing.md` — uses `WorkerNode` rows seeded here.
- `spec/19-main-worker-service/06-auth-and-2fa.md` — JWT verification uses `JwtPublicKeyPem` from §3.2.
- `spec/19-main-worker-service/07-core-api-endpoints.md` — §2.5 `/Workers/Register` and `/Workers/{Id}/Heartbeat`.
