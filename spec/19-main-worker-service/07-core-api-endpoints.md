...
| `CorrelationId` | The inbound `X-Correlation-Id` header. The middleware MUST reject the PATCH if absent (per `spec/04-database-conventions/07-rest-api-format.md`). |
...
- Header conventions: `spec/04-database-conventions/07-rest-api-format.md` §X-Auth-Action
