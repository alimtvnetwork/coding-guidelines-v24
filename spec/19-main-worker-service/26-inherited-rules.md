...
**Conflict rule:** if any of the above conflicts with `09-error-contract.md`, **`spec/03-error-manage/` wins** for the rule itself; the envelope shape is owned by `08-§2`.
...
- `09-error-contract.md` §1 — names `spec/03-error-manage/` as the source of catch/log/rethrow.
- `16-tunable-constants.md` §4 — names `spec/06-seedable-config-architecture/` as the source of seed semantics.
- `12-split-db-tier-reconciliation.md` §5 — names `spec/05-split-db-architecture/` as the source of the tier model.
