...
> See [08-split-db-pattern.md](./08-split-db-pattern.md) for the full Split DB specification including directory layout, DB registry, cross-domain rules, and migration strategy.
...
**Seeding pattern** — see [`15-rbac-and-status-seed.md`](../19-main-worker-service/15-rbac-and-status-seed.md) for the canonical seed format used by `19-main-worker-service`.
