...
| Database naming | [./03-naming-conventions.md](./03-naming-conventions.md) |
...
| `X-Idempotency-Key` | All `POST`, `PUT`, `PATCH` | ULID (26 chars), max 64 chars | Server stores result for `IdempotencyKeyTtlSeconds` (per `spec/19-main-worker-service/16-tunable-constants.md` §2.2). Replay returns cached result. |
...
- `spec/19-main-worker-service/07-core-api-endpoints.md` §1 — first introduced these headers.
- `spec/19-main-worker-service/16-tunable-constants.md` §2.2 — TTL + max-length values.
- `spec/19-main-worker-service/14-error-codes.md` — error codes for header-validation failures.
