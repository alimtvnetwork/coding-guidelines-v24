...
__INSTALLER_LOG_FILE="$__INSTALLER_LOG_DIR/splitdb-install-$(date -u +%Y%m%dT%H%M%SZ).log"
{
    echo "# splitdb-install crash log"
...
__installer_on_err() {
    ...
    local cmd=${2:-?}
    {
        echo ""
        echo "════════════════════════════════════════════════════════"
        echo "  ❌ splitdb-install FAILED (exit $rc) at line $line"
        echo "     command: $cmd"
        echo "  ────────────────────────────────────────────────────"
...
BUNDLE_NAME="splitdb"
BUNDLE_MAPPING="02-spec/04-database-conventions|02-spec/04-database-conventions 02-spec/05-split-db-architecture|02-spec/05-split-db-architecture 02-spec/06-seedable-config-architecture|02-spec/06-seedable-config-architecture version.json|version.json .ai-memory/what-to-read.md|.ai-memory/what-to-read.md .ai-memory/memory|.ai-memory/memory"
