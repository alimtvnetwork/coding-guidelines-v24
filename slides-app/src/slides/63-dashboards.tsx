    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        ...
        <ActionPanel
          ...
          rule="Every deployable service (api, worker, scheduler, gateway) ships a golden-signals dashboard at path `docs/dashboards/{service}.json` in the repo, provisioned automatically by CI to the observability stack. The dashboard has exactly five panels, always in this order and with these units: (1) Traffic, requests per second, broken down by route; (2) Errors, 5xx per second AND 5xx as percent of total, broken down by route; (3) Latency, p50 + p95 + p99 in milliseconds, broken down by route; (4) Saturation, CPU percent + memory percent + primary-pool utilisation percent; (5) one Domain panel, the single KPI that defines 'is this service doing its job' (queue depth for a worker, cache hit rate for a cache, checkout completion rate for checkout). Every panel has a description field naming the SLI, the alert that watches it (OPS-002), and the runbook step that responds to it (OPS-003). Dashboards are code, reviewed in PRs, versioned in git; changes go through the same PR template (WF-004) as source code."
