    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        ...
        <ActionPanel
          ...
          rule="spec/17/31 line 110 is absolute. Two parts. (1) Every composite type gets a name. Composite means: any object type with 2+ fields, any `Array<...>` of a composite, any `Map`/`Set`/`Record`/`Promise` of a composite, any union of 2+ non-primitive members, any function type used more than once. `type UsersById = ReadonlyMap<UserId, readonly User[]>` gets declared once and used everywhere. `Map<string, Array<{ id: number; name: string }>>` inline is banned. (2) Every generic parameter gets a meaningful name in application code. `TItem`, `TKey`, `TResponse`, `TError`, `TProps`, `TState`, `TAction`, `TDeps`, `TResult`. Never bare `T`, `U`, `K`, `V`. Utility library code (fp-ts, ts-toolbelt) is the only exception, and this codebase is not that. The `T` prefix disambiguates generics from concrete types in call-site hovers. Domain aliases pair with NAM-001 (PascalCase for types) and REACT-009 (no anonymous shapes)."
