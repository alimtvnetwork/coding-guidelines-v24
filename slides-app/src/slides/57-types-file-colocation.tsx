    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        <CodeDiff
          ...
          before={BEFORE}
          after={AFTER}
          beforeLabel="Bad. Anonymous prop object literal on the signature means: (1) no name to import for tests, Storybook stories, or wrapper components; (2) IDE hover shows the raw literal instead of `ProfileCardProps`, so the shape reads as noise; (3) the `user` shape is spelled inline AND drifts from the app-wide `User` type in `src/types/user.ts`; (4) `onSave` accepts a different, narrower object than `user`, guaranteeing an impedance mismatch when a caller passes the full user back; (5) adding one prop grows the component signature by 3-5 lines and pushes the file toward the REACT-007 100-line cap for no reason."
          afterLabel="Good. `ProfileCardProps` lives in a sibling `types.ts` and is imported by the component, the tests, the Storybook story, and any wrapper. `user` and the `onSave` payload both use the shared `User` type from `src/types/user.ts`, so there is one source of truth. `index.ts` re-exports the component and the prop type as one public surface. Adding a prop is a one-line change in `types.ts` and the component signature never grows past `ProfileCardProps`."
        />
        <ActionPanel
          slideId="57-types-file-colocation"
          symptom="`ProfileCard` had 11 props inlined on its signature (34-line function signature, 47-line file). Three ripple failures: (1) the Storybook story redeclared the prop shape with slightly different optionality (`email?: string` vs `email: string`), so the story rendered a state that could not exist in the app; (2) a wrapper `EditableProfileCard` retyped the same 11 props from scratch and got two wrong; (3) a rename from `avatarUrl` to `AvatarUrl` for NAM-001 alignment required editing 6 files instead of 1, and one file was missed and shipped a runtime `undefined` in prod. All three trace to one root cause: the prop shape had no name and no home."
          rule="spec/17/31 line 111 is absolute. Prop types and event handler types live in a dedicated `types.ts` next to the component. Shared cross-component types (`User`, `Order`, `AuditEvent`) live in `src/types/‹domain›.ts` and are imported. Never inline an anonymous object type on a component signature: `({ user, onSave }: { user: User; onSave: (u: User) =› void })` is wrong, `({ user, onSave }: ProfileCardProps)` is right. The pattern is one folder per non-trivial component: `src/components/profile-card/{ProfileCard.tsx, types.ts, index.ts}` (plus `ProfileCard.test.tsx`, `ProfileCard.stories.tsx` when relevant). Trivial single-use components (one prop, one line, zero handlers) may inline, but the moment a second prop or a handler lands, extract. Pairs with REACT-009 (no tuples), REACT-010 (name every composite), and REACT-007 (files under 100 lines) as one system: the types.ts file is where extracted names go so the component file stays small."
