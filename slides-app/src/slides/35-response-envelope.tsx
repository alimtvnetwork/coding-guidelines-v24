    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        ...
        <ActionPanel
          ...
          rule="Every backend response uses the envelope `{ data, errors[], meta }`. `errors` is always an array (empty on success), each item carries a registered `ErrorCode`, a human message, and optional field. `meta` always carries `requestId`. The frontend has exactly one `apiCall<T>` helper that parses the envelope, logs `errors` with `requestId` and `url` context, and throws an `AppError` with the first code so UI code can switch on it. Callers do not read `.errors` directly. Per spec/17/31 line 80 and the error-management digest."
          doThis="Create (or extend) `src/lib/api.ts` with `apiCall<T>` and the `Envelope<T>` type. On the backend, add a middleware/response helper that wraps every handler so success and error paths always produce the same envelope with a `requestId`. Migrate callers one feature at a time: delete per-caller try/catch, replace `fetch(...).then(r => r.json())` with `await apiCall<Order[]>(...)`. Reject any new route or caller that bypasses the envelope or the helper."
        />
      </div>
    </SlideLayout>
  );
