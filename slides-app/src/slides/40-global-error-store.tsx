    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        ...
        <ActionPanel
          ...
          rule="Every user-facing error goes through ONE global error store fed by the shared `apiCall` parser (ERR-005). The store holds at most one active error, tagged with `code`, `requestId`, and `at`. A SINGLE `<ErrorModal>` mounted at the app root reads the store and renders it, with copy chosen by `code` from a shared registry (ERR-004). Feature components never call `toast.error`, never keep their own `useState<string|null>` for errors, and never render their own alerts. On route change the store clears automatically. Every `push` logs at `error` level with the full context per LOG-002. Per spec/17/31 line 80."
          doThis="Create `src/lib/errorStore.ts` with `current`, `push(e)`, and `clear()`. Wire `apiCall` (ERR-005) to call `errorStore.getState().push(parseError(body))` on failure, with `code` from the ERR-004 registry and `requestId` from the response envelope. Mount `<ErrorModal>` exactly once in `src/app/AppRoot.tsx`, above the router. Add a router subscription that calls `clear()` on route change. Add an ESLint rule (or CI grep) banning `toast.error(` and `useState<string|null>(null)` named `error`/`err` outside `src/lib/errorStore.ts` and `src/app/ErrorModal.tsx`. Migrate existing feature components in one PR, delete their local alerts, and confirm the single modal receives the failure in DevTools."
        />
      </div>
    </SlideLayout>
  );
