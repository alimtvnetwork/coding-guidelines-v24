    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
        ...
        <ActionPanel
          ...
          rule="spec/17/31 line 108 is absolute for any function that calls another hook. Three parts, all required: (1) name starts with `use` (`useOrder`, `useDebouncedValue`, `useFocusTrap`), which is how `react-hooks/rules-of-hooks` and `react-hooks/exhaustive-deps` find the call site and how humans know a function is stateful; (2) return a named `type` or `interface` with named fields (`OrderQueryResult`, `DebouncedValue<T>`), never a bare tuple (see REACT-009 for the general rule against tuples as public shapes); (3) hook calls at the top level only, in the same order every render. Never gate a `useState`/`useEffect`/`useMemo`/`useCallback`/`useContext`/`useRef` call on `if`, `&&`, `?:`, `try`, `switch`, `for`, or `while`. The guard belongs inside the hook body (`useEffect(() => { if (!id) return; ... }, [id])`), never around the hook call. Custom hooks compose custom hooks under the same rules."
