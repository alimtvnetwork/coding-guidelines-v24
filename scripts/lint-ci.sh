...
STEPS=(
  ...
  "Run Axios version check|python3 linter-scripts/check-axios-version.py"
  "Check spec folder references (stale-link guard)|python3 linter-scripts/check-spec-folder-refs.py"
  "Check forbidden spec paths (re-split + merge-proposal guard)|python3 linter-scripts/check-forbidden-spec-paths.py"
  ...
  "Check Lovable prompts loaded (index ↔ 01-prompts/* sync)|python3 linter-scripts/check-prompts-loaded.py"
  ...
  "Check runner dispatch anti-patterns (run.sh / run.ps1)|python3 linter-scripts/check-runner-dispatch-antipatterns.py"
