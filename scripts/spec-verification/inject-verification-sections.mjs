 ...
 *  - Skips: ``readme.md``, ``97-acceptance-criteria.md``,
...
function printHelp() {
  ...
  --mode <m>      'overview-only' (default) — inject only into each folder's
                  readme.md. 'all-files' — inject into every prose file.
...
function shouldSkip(fullPath, relPath, mode) {
  ...
  // Spec-root files (e.g. 02-02-spec/readme.md) own their own bespoke
  // verification logic — never touch them.
  if (relPath.split(sep).length === 1) return true;
  // overview-only mode: only touch each folder's readme.md.
  if (mode === "overview-only") {
    if (base !== "readme.md") return true;
    // Only the top-level overview (e.g. 02-spec/04-database-conventions/readme.md
    // — depth 2). Skip nested readme.md files in subfolders.
...
function main() {
  ...
  for (const full of files) {
    ...
    if (args.strip) {
      ...
      // readme.md files lost their per-file block earlier and are
      // now treated like any other prose file.)
      if (args.mode === "overview-only" && base === "readme.md" && rel.split(sep).length === 2) {
        summary.skipped++; continue;
      }
    }
