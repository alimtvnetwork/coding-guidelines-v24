...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/splitdb-install.ps1))) -Version v6.43.0 -Target .\vendor
#>
...
$Script:__InstallCrashLogFile = Join-Path $Script:__InstallCrashLogDir ("splitdb-install-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ") + ".log")
...
Write-InstallLog "# splitdb-install crash log"
...
$BundleName = "splitdb"
$BundleMapping = "02-spec/04-database-conventions|02-spec/04-database-conventions,02-spec/05-split-db-architecture|02-spec/05-split-db-architecture,02-spec/06-seedable-config-architecture|02-spec/06-seedable-config-architecture,version.json|version.json,.ai-memory/what-to-read.md|.ai-memory/what-to-read.md,.ai-memory/memory|.ai-memory/memory"
...
Write-Host "  📦 splitdb-install v6.43.0" -ForegroundColor Cyan
...
function Copy-Mapping {
    ...
    try {
        ...
        $summaryData = [ordered]@{ bundle = "splitdb"; version = $finalGuidelineVersion; previousVersion = $prevVersion; installedAt = (Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"); filesInstalled = @($installedFilesSorted); filesRemoved = @($removedFilesSorted) }
...
Restore-CallerPreferences
    ...
    Write-Host "  ❌ splitdb-install failed (code $code)" -ForegroundColor Red
