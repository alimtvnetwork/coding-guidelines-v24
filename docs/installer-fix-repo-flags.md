  ...
  install-and-fix-repo:
    ...
    steps:
      ...
      - name: Run installer + fix-repo (Bash)
        ...
        run: |
          set -euo pipefail
          curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/install.sh \
            | bash -s -- \
                --run-fix-repo \
                --rollback-on-fix-repo-failure \
      ...
      - name: Run installer + fix-repo (PowerShell)
        ...
        run: |
          ...
          irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/install.ps1 `
            -OutFile install.ps1
          .\install.ps1 `
            -RunFixRepo `
            -RollbackOnFixRepoFailure `
