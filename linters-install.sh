...
__INSTALLER_LOG_FILE="$__INSTALLER_LOG_DIR/linters-install-$(date -u +%Y%m%dT%H%M%SZ).log"
{
    echo "# linters-install crash log"
...
__installer_on_err() {
    ...
    local cmd=${2:-?}
    {
        echo ""
        echo "════════════════════════════════════════════════════════"
        echo "  ❌ linters-install FAILED (exit $rc) at line $line"
        echo "     command: $cmd"
        echo "  ────────────────────────────────────────────────────"
...
BUNDLE_NAME="linters"
BUNDLE_MAPPING="linters|linters linters-cicd|linters-cicd version.json|version.json .ai-memory/what-to-read.md|.ai-memory/what-to-read.md .ai-memory/memory|.ai-memory/memory"
