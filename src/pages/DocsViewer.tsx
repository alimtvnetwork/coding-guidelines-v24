...
function useDocsViewerState(allFiles: SpecNode[]) {
  ...
  return { activeFile, searchQuery, setSearchQuery, searchOpen, setSearchOpen, handleSelect, handleSearchSelect, openSearch };
}
  ...
  return (
    <div className="min-h-screen flex flex-col w-full">
      <div className="flex-1 flex w-full">
        <DocsContent activeFile={state.activeFile} allFiles={allFiles} tree={tree} onSelect={state.handleSelect} searchQuery={state.searchQuery} setSearchQuery={state.setSearchQuery} onSearchOpen={state.openSearch} />
      </div>
      <DocsViewerFooter />
    </div>
  );
