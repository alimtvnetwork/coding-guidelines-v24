...
2. **Recent 20-Task Compact Tracking Register:** Maintain a compact, accurate mental and written model of completed vs pending work by introducing the `Recent Completed Tasks Register (Last 20 Tasks)` table in `.ai-memory/plans/readme.md`.
3. **Cross-Referencing & Continuity:** Synchronize `.ai-memory/what-to-read.md` with the 30-commit audit and the 20-task register in `.ai-memory/plans/readme.md`. Ensure that every new memory write refers back to this register and `what-to-read.md` so that during loop executions, subsequent agents maintain seamless continuity with recent progress.
...
- `01-prompts/03-read-write/03-write-memory.md`:
  ...
  - Added Pre-Flight Step 0 and updated item 5 to verify the 20-task register in `.ai-memory/plans/readme.md`.
...
- `.ai-memory/what-to-read.md`:
  ...
  - Linked the Recent Completed Tasks Register in `.ai-memory/plans/readme.md`.
...
2. **Bounded 20-Task Rolling Window:** Rather than scanning hundreds of historical change logs or leaving the agent guessing, a compact 20-task register in `.ai-memory/plans/readme.md` provides an instant snapshot of completed work without consuming excessive token context.
3. **Bi-Directional Linkage:** By cross-linking `what-to-read.md` and `plans/readme.md`, read prompts naturally absorb recent task history, and write prompts update and maintain that history.
