...
func TestUnmarshalIntegerJSON_ValidString(t *testing.T) {
	...
	err := baseenumer.UnmarshalIntegerJSON([]byte(`"Active"`), &target, "test", testByteMap, 2, testByteInvalid)
	if err != nil || target != testByteActive {
		t.Fatalf("failed to unmarshal string: %v, got %v", err, target)
	}

	err = baseenumer.UnmarshalIntegerJSON([]byte(`"inactive"`), &target, "test", testByteMap, 2, testByteInvalid)
...
func TestUnmarshalIntegerJSON_Numeric(t *testing.T) {
	var target testByteEnum
	err := baseenumer.UnmarshalIntegerJSON([]byte(`1`), &target, "test", testByteMap, 2, testByteInvalid)
	if err != nil || target != testByteActive {
		t.Fatalf("failed to unmarshal numeric literal: %v, got %v", err, target)
	}

	err = baseenumer.UnmarshalIntegerJSON([]byte(`"2"`), &target, "test", testByteMap, 2, testByteInvalid)
	if err != nil || target != testByteInactive {
...
func TestUnmarshalIntegerJSON_NullAndEmpty(t *testing.T) {
	...
	err := baseenumer.UnmarshalIntegerJSON([]byte(`null`), &target, "test", testByteMap, 2, testByteInvalid)
...
func TestUnmarshalIntegerJSON_OutOfRange(t *testing.T) {
	var target testByteEnum
	err := baseenumer.UnmarshalIntegerJSON([]byte(`99`), &target, "test", testByteMap, 2, testByteInvalid)
	if err == nil {
		t.Fatalf("expected error on out of range numeric 99, got nil")
	}

	err = baseenumer.UnmarshalIntegerJSON([]byte(`"UnknownVariant"`), &target, "test", testByteMap, 2, testByteInvalid)
	if err == nil {
...
func TestUnmarshalStringJSON_Valid(t *testing.T) {
	...
	err := baseenumer.UnmarshalStringJSON([]byte(`"Pending"`), &target, "test", testStringMap, testStringUnknown)
	if err != nil || target != testStringPending {
		t.Fatalf("failed to unmarshal valid string: %v, got %v", err, target)
	}

	err = baseenumer.UnmarshalStringJSON([]byte(`"running"`), &target, "test", testStringMap, testStringUnknown)
...
func TestUnmarshalStringJSON_NullAndErrors(t *testing.T) {
	...
	err := baseenumer.UnmarshalStringJSON([]byte(`null`), &target, "test", testStringMap, testStringUnknown)
	if err != nil || target != testStringUnknown {
		t.Fatalf("failed on null: %v, got %v", err, target)
	}

	err = baseenumer.UnmarshalStringJSON([]byte(`"NonExistent"`), &target, "test", testStringMap, testStringUnknown)
	if err == nil {
...
func (b *testByteEnum) UnmarshalJSON(data []byte) error {
	return baseenumer.UnmarshalIntegerJSON(data, b, "testByte", testByteMap, 2, testByteInvalid)
}
