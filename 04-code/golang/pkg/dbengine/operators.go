...
func (o SqlOperator) ToJSON() (string, *appfault.AppError) {
	b, err := json.Marshal(string(o))
	if err != nil {
		return "", appfault.NewAppBuilder(errtype.Serialization, "serialize sql operator to json").SetCause(err).Build()
	}
...
func (o *SqlOperator) FromJSON(s string) *appfault.AppError {
	var str string
	if err := json.Unmarshal([]byte(s), &str); err != nil {
		return appfault.NewAppBuilder(errtype.Serialization, "deserialize sql operator from json").SetCause(err).Build()
	}

	target := SqlOperator(str)
	if !sqlOperatorValidMap[target] {
		return appfault.NewAppBuilder(errtype.Validation, fmt.Sprintf("invalid sql operator: %s", str)).Build()
	}
...
func (r sqlOperatorRegistry) ToJSON() (string, *appfault.AppError) {
	b, err := json.Marshal(r.Names())
	if err != nil {
		return "", appfault.NewAppBuilder(errtype.Serialization, "serialize sql operator registry to json").SetCause(err).Build()
	}
