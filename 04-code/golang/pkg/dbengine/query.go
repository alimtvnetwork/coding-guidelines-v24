...
func (b *QueryBuilder[T, F]) SetError(err *appfault.AppError) *QueryBuilder[T, F] {
	if b.err == nil {
		b.err = err
	}
...
func (b *QueryBuilder[T, F]) Err() *appfault.AppError {
	return b.err
}
...
// ViewManager defines methods for creating or reusing database views.
...
func (b *QueryBuilder[T, F]) CreateViewOrUseView(ctx context.Context, viewName string, requiredColumns ...string) BoolResult {
	...
	vc, isViewManager := b.repo.db.(ViewManager)
	if !isViewManager {
		return FailureBool(appfault.New(errtype.Validation, "executor does not support view creation"))
	}
...
func (b *QueryBuilder[T, F]) First(ctx context.Context) EntityResult[T] {
	...
	if scanErr != nil {
		return FailureEntity[T](appfault.Wrap(errtype.Database, scanErr, "scan first "+b.repo.tableName))
	}
...
func (b *QueryBuilder[T, F]) FindAll(ctx context.Context) ListResult[T] {
	...
	for rows.Next() {
		...
		if scanErr != nil {
			return FailureList[T](appfault.Wrap(errtype.Database, scanErr, "scan row "+b.repo.tableName))
		}
...
func (b *QueryBuilder[T, F]) Count(ctx context.Context) Int64Result {
	...
	if scanErr != nil {
		return FailureInt64(appfault.Wrap(errtype.Database, scanErr, "count "+b.repo.tableName))
	}
...
func (b *QueryBuilder[T, F]) Delete(ctx context.Context) RowsAffectedResult {
	...
	if err != nil {
		return FailureRowsAffected(appfault.Wrap(errtype.Database, err, "get rows affected for delete "+b.repo.tableName))
	}
