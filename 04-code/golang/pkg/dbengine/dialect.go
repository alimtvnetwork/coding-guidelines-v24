...
// Variant represents any dynamic value acceptable for comparisons.
type Variant = any
...
func (d DbType) ToJSON() (string, *appfault.AppError) {
	b, err := json.Marshal(string(d))
	if err != nil {
		return "", appfault.NewAppBuilder(errtype.Serialization, "serialize db type to json").SetCause(err).Build()
	}
...
func (d *DbType) FromJSON(s string) *appfault.AppError {
	var str string
	if err := json.Unmarshal([]byte(s), &str); err != nil {
		return appfault.NewAppBuilder(errtype.Serialization, "deserialize db type from json").SetCause(err).Build()
	}

	target := DbType(str)
	if !dbTypeValidMap[target] {
		return appfault.NewAppBuilder(errtype.Validation, fmt.Sprintf("invalid db type: %s", str)).Build()
	}
...
func (r dbTypeRegistry) ToJSON() (string, *appfault.AppError) {
	b, err := json.Marshal(r)
	if err != nil {
		return "", appfault.NewAppBuilder(errtype.Serialization, "serialize db type registry to json").SetCause(err).Build()
	}
...
func ResolveCompiler(dialect DatabaseDialectType) (DialectCompiler, *appfault.AppError) {
	switch dialect {
	...
	default:
		return nil, appfault.NewAppBuilder(errtype.Validation, fmt.Sprintf("unsupported dialect: %s", dialect)).Build()
	}
}
