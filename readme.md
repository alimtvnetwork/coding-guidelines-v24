<p align="center">
  <a href="https://github.com/alimtvnetwork/coding-guidelines-v24">
    <img
...
<p align="center">
  ...
  <!-- STAMP:FILES -->699<!-- /STAMP:FILES -->
  <!-- STAMP:LINES -->149,116<!-- /STAMP:LINES -->
</p>
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/install.ps1))) -n
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/install.sh | bash -s -- -n
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/linters-install.sh | bash
```
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/cli-install.ps1 | iex
```
...
```bash
curl -fsSL https://github.com/alimtvnetwork/coding-guidelines-v24/releases/download/v5.131.0/install.sh | bash -s -- -v v5.131.0
```
...
When this repo bumps its major version (e.g. `coding-guidelines-v24` → `coding-guidelines-v24`), use `fix-repo` to rewrite prior versioned-repo-name tokens across all tracked text files. The script auto-detects the base name and current version from `git remote get-url origin`, nothing is hardcoded.
...
Token form: `{RepoBase}-v{N}` (e.g. `coding-guidelines-v24`). URLs are preserved automatically, only the token segment changes. A numeric-overflow guard prevents `coding-guidelines-v24` from matching inside `coding-guidelines-v170`. Full normative spec: [`spec-authoring/22-fix-repo/01-spec.md`](spec-authoring/22-fix-repo/01-spec.md).
...
- Parser self-test guards the guard. `scripts/tests/print-required-checks.test.mjs` runs 13 assertions covering the two known parseWorkflow traps (top-level `concurrency:` leaking into last job, missing `jobs:` section) plus real-workflow parse coverage. Refactor exported `parseWorkflowText` + `checkStale` and guarded `main()` so imports don't exit the process. Prevents the class of "silent-pass because parser returned empty" regression that let v5.117 ship strict guards over zero baselines.
...
- Release preflight parity. `scripts/release.mjs` now runs `sync-guidelines.mjs --check`, `validate-slides-sra.mjs`, and `slides-app` `tsc -b` before the version bump: guideline drift, an SRA-shaped violation, or a slides-app type error now fails the release immediately instead of shipping a broken deck. `.husky/pre-push` mirrors the same two validators after the lint-ci fast subset, so contributors get the identical signal locally before push.

### What's new in v5.114.0

- Polish pass. Presenter view (`slides-app/src/App.tsx` `PresenterView`) gains a section filter-chip toolbar in the notes column: one pill per chapter (10 chips) showing label + slide-count, current chapter's chip highlighted via `aria-pressed`, clicking any chip jumps to that chapter's first slide via a new `onJump` prop wired to `goto(n)`. Chips wrap and scroll (`max-height: 128px`) so long chapter lists never push the timer off-screen. Print handout (`?print` `PrintView` + `styles/slide.css` `.print-page`) gets a density pass: on-screen preview now renders each 1920x1080 page at `transform: scale(0.5)` with `-540px` negative margin so a reviewer scrolling 67 slides sees 4x more per viewport, gap tightened from 24px to 12px, box-shadow lightened; a new bottom-left `.print-page-stamp` chrome overlay prints `NN / total · RULE-ID · title` in a mono pill so PDF handouts self-identify per page; `@media print` restores native scale, removes the negative margin, and darkens the stamp for ink-on-paper contrast. No content or slide-authoring API changes.

### What's new in v5.113.0

- Polish pass. Closing slide (`slides-app/src/slides/12-closing.tsx`) refreshed from the legacy 11-item list to a 12-item rule-id keyed action grid (MUST-002, NAM-001, BOOL-002, SIZE-001, CF-001, ERR-001, LOG-002, SCHEMA-002, REACT-002, REACT-011, A11Y-001, OPS-002) that spans all 10 sections of the completed 67-slide catalogue; each row shows a monospace rule pill so learners can trace items back to their rule slide. Command palette (Cmd/Ctrl+K) gains section jump: prefix `s:<name>` or `#<name>` filters to sections only (`s:react`, `#ops`), and with no prefix section entries render inline above slide matches with a `§` marker plus slide-count, so the palette doubles as a mini-TOC; selecting a section lands on its first slide. Fixed a stray inline-backtick bug in the OPS-002 and OPS-003 template literals (unescaped `` ` `` around shell commands prematurely terminated the surrounding template string).
...
- SS-02 tasks 62-63: Workflow chapter continues. Slide 60 `WF-003` (hard) codifies the test pyramid: 80 percent unit (pure functions, ms), 15 percent integration (one seam each, s), 5 percent e2e (browser wiring proofs, tens of s); CI enforces per-layer timers, `scripts/report-test-shape.mjs` caps e2e at 10 percent of total specs, and `src/pricing`, `src/auth`, `src/schema` require 100 percent line coverage. Slide 61 `WF-004` (hard) turns `.github/pull_request_template.md` into a merge gate with 8 machine-checkable items: linked issue, one-sentence root cause before fix, WF-003 test layer, SCHEMA-006 ERD when DB touched, A11Y-001 keyboard + axe when UI touched, CHANGELOG entry, no swallowed errors, size caps, no placeholder types; enforced by `pr-template-check` action and `danger.js`, waivers logged to `docs/waivers.md` and reviewed monthly in the RCA-002 retro. 63 slides validated, SRA green.
...
- **Root-cause-first workflow slide (SS-02 task 17).** New [`slides-app/src/slides/14-root-cause-first.tsx`](slides-app/src/slides/14-root-cause-first.tsx) (ruleId `MUST-002`, hard) contrasts the RCA-first loop (Reproduce → Trace → State the cause → Minimum fix → Prove it) against the symptom-patch loop (try/catch, fallback, extra effect, ship). Operationalizes non-negotiable #2 from `MUST-001`. SRA validator now passes 16 slides.
...
### What's new in v5.62.0

- **Print handout route (SS-02 task 9).** Append `?print` to the slides-app URL to render every slide stacked at native 1920x1080; then Cmd/Ctrl+P > Save as PDF produces a landscape handout that matches the on-screen design. New `PrintView` in [`slides-app/src/App.tsx`](slides-app/src/App.tsx) plus `@page { size: 1920px 1080px; margin: 0 }` in [`slides-app/src/styles/slide.css`](slides-app/src/styles/slide.css). Enables offline review, distribution to non-technical stakeholders, and a future `slides-handout.pdf` release asset.
...
- [Milestone 02: AppFault Result Monad, Dynamic Conversions & Verification Systems](.lovable/plans/completed/02-appfault-result-monad-and-verification-systems.md)
- [Milestone 03: File Utilities, PathInfo & Modular Enum Architecture](.lovable/plans/completed/03-fileutil-pathinfo-and-enum-architecture.md)
