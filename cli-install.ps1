...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/cli-install.ps1))) -Version v6.46.0 -Target .\vendor
#>
...
$Script:__InstallCrashLogFile = Join-Path $Script:__InstallCrashLogDir ("cli-install-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ") + ".log")
...
Write-InstallLog "# cli-install crash log"
...
Write-Host "  📦 cli-install v6.46.0" -ForegroundColor Cyan
...
Restore-CallerPreferences
    ...
    Write-Host "  ❌ cli-install failed (code $code)" -ForegroundColor Red
