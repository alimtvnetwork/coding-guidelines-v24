...
When the user's input clearly belongs to a feature, mirror the cleaned conversation to `/conversation/xx-feature/xx-title-of-conv.md` and update `01-prompts/16-rewrite/01-conversation-log.md`. `xx` is a two-digit sequence. This is the ONLY archive location for user conversation content. Never scatter copies elsewhere.
