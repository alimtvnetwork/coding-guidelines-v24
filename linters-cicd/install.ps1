...
# NOTE: Help is handled BEFORE param parsing / network probes so that
#   `install.ps1 --help` (or `-h`) never triggers a remote version lookup.
...
# Pre-parse help flags from raw args so long-form `--help` works even though
# PowerShell `param()` would otherwise reject it as an unknown argument.
if ($args) {
    foreach ($a in $args) {
        if ($a -in @('-h', '--help', '/?')) { Show-Usage }
    }
}
