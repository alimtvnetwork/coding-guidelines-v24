#!/usr/bin/env python3
"""CODE-RED-014 - Strict Boolean Conditions. Python."""
    ...
    for i, raw in enumerate(text.splitlines(), start=1):
        ...
        if ":" in line and "if " in line:
            ...
            if and_count + or_count > 1:
                ...
                left_neg = "not " in left
                right_neg = "not " in right
...
def main() -> int:
    args = build_parser("CODE-RED-014 strict-boolean-conditions (Python)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-strict-bool-py", tool_version="1.0.0", rules=[RULE])
