#!/usr/bin/env python3
"""CODE-RED-012 - Boolean Return Wrapper. TypeScript."""
...
FUNC_MULTI_RET_BOOL_RE = re.compile(r"(?:function\s+[A-Za-z0-9_]+\s*|^[A-Za-z0-9_]+\s*|\s*)\([^)]*\)\s*:\s*\[[^\]]*?(?:,[^\]]*\bboolean\b|\bboolean\b[^\]]*,)[^\]]*?\]\s*(?:\{|=>)")
        ...
        if FUNC_MULTI_RET_BOOL_RE.search(line):
            findings.append(
                Finding(
                    rule_id=RULE.id,
                    level="error",
                    message="Tuple multi-return contains a raw boolean. Use a wrapper interface/object.",
...
def main() -> int:
    args = build_parser("CODE-RED-012 boolean-return-wrapper (TS)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-bool-ret-ts", tool_version="1.0.0", rules=[RULE])
