#!/usr/bin/env python3
"""CODE-RED-010 - Enum suffix. Python. Flags enums that don't end with _type."""
            ...
            if not name.endswith("_type"):
                findings.append(
                    Finding(
                        rule_id=RULE.id,
                        level="error",
                        message=f"Enum class '{name}' must end with '_type' suffix.",
...
def main() -> int:
    args = build_parser("CODE-RED-010 enum-suffix (Python)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-enum-suffix-python", tool_version="1.0.0", rules=[RULE])
