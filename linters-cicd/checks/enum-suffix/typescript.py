#!/usr/bin/env python3
"""CODE-RED-010 - Enum suffix. TypeScript. Flags enums that don't end with Type."""
            ...
            if not name.endswith("Type"):
                findings.append(
                    Finding(
                        rule_id=RULE.id,
                        level="error",
                        message=f"Enum '{name}' must end with 'Type' suffix (e.g., {name}Type).",
...
def main() -> int:
    args = build_parser("CODE-RED-010 enum-suffix (TypeScript)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-enum-suffix-ts", tool_version="1.0.0", rules=[RULE])
