#!/usr/bin/env python3
"""CODE-RED-013 - Inverted Complex Conditions. PHP."""
...
INVERTED_COMPLEX_RE = re.compile(r"if\s*\(\s*!\s*\([^)]+(&&|\|\||\band\b|\bor\b)[^)]+\)\s*\)")
...
def main() -> int:
    args = build_parser("CODE-RED-013 inverted-complex-conditions (PHP)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-invert-cond-php", tool_version="1.0.0", rules=[RULE])
