#!/usr/bin/env python3
"""CODE-RED-013 - Inverted Complex Conditions. Rust."""
...
INVERTED_COMPLEX_RE = re.compile(r"if\s+!\s*\([^)]+(&&|\|\|)[^)]+\)\s*\{")
...
def main() -> int:
    args = build_parser("CODE-RED-013 inverted-complex-conditions (Rust)").parse_args()
    _globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(tool_name="coding-guidelines-invert-cond-rust", tool_version="1.0.0", rules=[RULE])
