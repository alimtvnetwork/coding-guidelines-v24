...
>    - Python AI Scripts: `.lovable/ai-fix-scripts/` (e.g. `01-file-manipulator.py`, `05-guideline-autofixer.py`, `06-cicd-local-runner.py`, `07-relative-path-fixer.py`).
...
- [ ] **Native Relative Path Fixer:** If you need to scan and fix absolute paths or `file:///` URIs, you MUST natively use `python .lovable/ai-fix-scripts/07-relative-path-fixer.py [path]` rather than writing a new script from scratch.
...
- [ ] **Index Documentation:** I have updated `.lovable/ai-fix-scripts/01-index.md` using sequential script naming (e.g., `07-relative-path-fixer.py`). For every script, I have included a `<details>` collapsible tag explaining exactly why the script is there and what it does.
...
3. **Autofixer Command:** `python .lovable/ai-fix-scripts/07-relative-path-fixer.py`
