...
3. **Linter & CI Verification:** Execute the section's dedicated linter and run `python .lovable/ai-fix-scripts/06-cicd-local-runner.py` ensuring all 19 gates pass 100% green (`exit 0`).
4. **Automated Plan Consolidation:** Run `python .lovable/ai-fix-scripts/20-plan-consolidator.py` to archive completed subtasks and update `.lovable/plans/01-index.md`.
