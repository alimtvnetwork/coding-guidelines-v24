...
| 5   | `.lovable/ai-fix-scripts/`                            | Automated Python utilities (`06-cicd-local-runner.py`, `08-fast-file-scanner.py`, `09-fast-cached-grep.py`, `01-index.md`). Pre-warms `tmp/` file caches for rapid zero-overhead file discovery. |
...
1. **Auto-Generate Antigravity Skills for Prompts:**
   - For every prompt ingested from `01-prompts-category/` or `.lovable/prompts/`, create or update a dedicated Antigravity skill in `.agents/skills/<slug>/skill.md` with standard YAML frontmatter (`name`, `description`).
