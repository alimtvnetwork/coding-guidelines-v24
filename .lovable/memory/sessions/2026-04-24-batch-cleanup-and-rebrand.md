...
1. **Slug rebrand** — replaced `coding-guidelines-v22` → `coding-guidelines-v22` → `coding-guidelines-v22` across the repo. Final canonical slug: **`alimtvnetwork/coding-guidelines-v22`**.
...
2. 🔴 **Slug must be `alimtvnetwork/coding-guidelines-v22` everywhere** — when doing repo-wide rebrand, run a full `grep -rn` sweep across **all** non-`.release` files. The user explicitly called out that the previous replacement was incomplete: *"you didn't do all replace in the repo"*.
...
(`coding-guidelines-v22`, `coding-guidelines-v22`..`v13`) inside the
...
3. The canonical, current slug remains
   **`alimtvnetwork/coding-guidelines-v22`** (see §Key decisions item 2)
