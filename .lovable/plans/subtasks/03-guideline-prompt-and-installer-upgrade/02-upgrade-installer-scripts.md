...
2. Enhance the PowerShell and Bash scripts logic for ersion.json merge:
   - For codingGuideline, calculate installedFiles array (by listing files extracted from the bundle).
   - Before extracting, read codingGuideline.installedFiles from the existing ersion.json. Any file in the old list that is NOT in the new list should be explicitly deleted.
