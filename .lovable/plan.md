...
- **Rebrand** `release-artifacts/release-install.{ps1,sh}` → `alimtvnetwork/coding-guidelines-v20`.
...
2. Canonical slug `alimtvnetwork/coding-guidelines-v20` everywhere — full `grep -rn` sweep after every rebrand. *"you didn't do all replace in the repo"* — never assume partial coverage is enough.
