...
- `slides-app/src/slides/59-a11y-floor.tsx`: the `rule` JSX attribute was a plain double-quoted string containing backslash-escaped inner quotes (`alt=\"\"`). JSX does not treat `\"` as an escape inside `"..."` attributes, so the parser closed the value at the first inner quote and produced TS1127 (invalid character) and TS1382 (unexpected token) that blocked `tsc -b`. Root cause: slide authored as if the attribute were a JS string literal. Fix: switched to a template literal `rule={`...`}` with bare `""` inside.
- `slides-app/src/deck/registry.ts`: added `"structure"` to the `SlideSection` union and a matching entry in `SECTIONS` ("Code Structure — Line-gaps, file/function size, DRY, immutability") so slides 26-33 type-check against the section literal instead of failing with TS2322.

### Added — Release preflight parity (release.mjs ↔ pre-push)

- `scripts/release.mjs` now runs three preflight checks before the version bump: `sync-guidelines.mjs --check`, `validate-slides-sra.mjs`, and `slides-app` `tsc -b`. Any failure exits with code 2 and leaves the working tree untouched, so a broken deck never gets tagged.
- `.husky/pre-push` mirrors the two script-only checks after the lint-ci fast subset, giving contributors the identical signal locally.

### Housekeeping

- Bumped `package.json` from `5.114.0` → `5.115.0`.
- Sync-managed artifacts regenerated by `npm run sync`: `version.json`, `src/data/specTree.json`, `public/health-score.json`, `readme.md`, `docs/architecture.md`.
...
- Bumped `package.json` from `5.108.0` → `5.109.0`.
...
- Bumped `package.json` from `5.97.0` to `5.98.0`.
- Sync-managed artifacts regenerated by `npm run sync`.
