...
.EXAMPLE
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/coding-guidelines-v24/main/error-manage-install.ps1))) -Version v6.44.0 -Target .\vendor
#>
...
$Script:__InstallCrashLogFile = Join-Path $Script:__InstallCrashLogDir ("error-manage-install-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ") + ".log")
...
Write-InstallLog "# error-manage-install crash log"
...
$BundleMapping = "02-spec/01-spec-authoring-guide|02-spec/01-spec-authoring-guide,02-spec/03-error-manage|02-spec/03-error-manage,version.json|version.json,.ai-memory/what-to-read.md|.ai-memory/what-to-read.md,.ai-memory/memory|.ai-memory/memory"
...
Write-Host "  📦 error-manage-install v6.44.0" -ForegroundColor Cyan
...
function Copy-Mapping {
    ...
    try {
        ...
        $summaryData = [ordered]@{ bundle = "error-manage"; version = $finalGuidelineVersion; previousVersion = $prevVersion; installedAt = (Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"); filesInstalled = @($installedFilesSorted); filesRemoved = @($removedFilesSorted) }
...
Restore-CallerPreferences
    ...
    Write-Host "  ❌ error-manage-install failed (code $code)" -ForegroundColor Red
